...
class SignupForm extends Component {
  ...
  registerUser = () =>
    authService
      ...
      .then(() => {
        this.setState({ registeredSuccesfully: true });
      })
      .catch(() =>
        this.setState({
          hasErrors: 'El email y password ingresados no estan registrados en nuestra base de datos.'
        })
      );
  ...
  handleSubmit = () => {
    if (
      ...
      this.setState({
        hasErrors: 'La contraseña ingresada debe tener entre 8 y 52 caracteres, y una letra y numero'
      });
    } else if (this.state.password !== this.state.confirmPassword) {
      this.setState({
        hasErrors: 'La contraseña ingresada debe coincidir con la confirmacion de la contraseña'
      });
